...
def profile_page(user_id):
    ...
    # Fetch technical skills and roles for the user
    user_skills = TechnicalSkill.query.filter_by(user_id=user.id).all()
    user_roles = UserRole.query.filter_by(user_id=user.id).all()

    # Collect skills and roles for query
    skill_names = [skill.name for skill in user_skills]
    role_names = [role.role_name for role in user_roles]

    # Prepare the clauses for job posting queries
    skill_clauses = [
        JobPosting.skills_required.ilike(f'%{skill_name}%') for skill_name in skill_names
    ]
    role_clauses = [
        JobPosting.project_type.ilike(f'%{role_name}%') for role_name in role_names
    ]
    description_clauses = [
        JobPosting.description.ilike(f'%{skill_name}%') for skill_name in skill_names
    ]

    # Combine the non-empty clauses using db.or_
    relevant_job_postings_query = JobPosting.query
    combined_clauses = skill_clauses + role_clauses + description_clauses
    if combined_clauses:
        relevant_job_postings_query = relevant_job_postings_query.filter(db.or_(*combined_clauses))

    relevant_job_postings = relevant_job_postings_query.all()
    ...
    return render_template(
        ...
        followed_freelancers=followed_freelancers, 
        is_following=is_following, 
        job_postings=relevant_job_postings, 
        skills=user_skills, 
        roles=user_roles
    )
...
UPLOAD_FOLDER = r'H:\Sophomore\4th_Sem\COMP202\freedev-3.2\app\uploads'
...
def search():
    ...
    filter_active = request.args.get('filter_active', type=int, default=0)
    ...
    if filter_active:
        return redirect(url_for('advanced_search', query=query))
...
def public_profile(email):
    ...
    # Only show follow/unfollow options if viewing someone else's profile and if the user's role is not "freelancer"
    show_follow_options = user.id != current_user_id and session.get('role') != 'freelancer'
...
def advanced_search():
    ...
    languages =  ["Java", "JavaScript", "Python", "C#", "C++", "Ruby", "PHP", "Swift", "Kotlin", "Go", "Rust", "TypeScript", "Shell", "R", "Scala", "Perl"]  
    frameworks = ["Django", "Flask", "Spring", "React", "Vue", "Angular", "Express.js", "Ruby on Rails", "Laravel", "ASP.NET", "Meteor.js", "Ember.js", "Backbone.js", "Knockout.js"]
    specialties = ['Web Developer', 'Data Scientist', 'Project Manager', 'Machine Learning Engineer', 'Cybersecurity Specialist', 'Network Engineer', 'Blockchain Developer', 'Cloud Solutions Architect', 'Embedded Systems Engineer', 'UI/UX Designer', 'App Developer', 'Systems Analyst']

    # Base query setups
    user_query = User.query.outerjoin(TechnicalSkill, User.id == TechnicalSkill.user_id)\
                            .outerjoin(UserRole, User.id == UserRole.user_id).filter(
        or_(
            User.name.ilike(f'%{query}%'),
            User.email.ilike(f'%{query}%'),
            UserRole.role_name.ilike(f'%{query}%'),
    ...
    job_postings_query = JobPosting.query.filter(
        or_(
            JobPosting.title.ilike(f'%{query}%'),
            JobPosting.description.ilike(f'%{query}%'),
            JobPosting.skills_required.ilike(f'%{query}%')
    ...
    # Create filter conditions for users
    user_filters = []
    for language in languages:
        if language in request.args.getlist('language'):
            user_filters.append(TechnicalSkill.name == language)
            active_filters.append(language)

    for framework in frameworks:
        if framework in request.args.getlist('framework'):
            user_filters.append(TechnicalSkill.name == framework)
            active_filters.append(framework)

    for specialty in specialties:
        if specialty in request.args.getlist('specialty'):
            user_filters.append(UserRole.role_name == specialty)
            active_filters.append(specialty)

    if user_filters:
        user_query = user_query.filter(or_(*user_filters))

    # Create filter conditions for job postings
    job_filters = []
    for language in languages:
        if language in request.args.getlist('language'):
            job_filters.append(JobPosting.skills_required.ilike(f'%{language}%'))

    for framework in frameworks:
        if framework in request.args.getlist('framework'):
            job_filters.append(JobPosting.skills_required.ilike(f'%{framework}%'))

    if job_filters:
        job_postings_query = job_postings_query.filter(or_(*job_filters))

    users = user_query.distinct().all()
    job_postings = job_postings_query.distinct().all()

    return render_template('advanced_search_results.html', users=users, job_postings=job_postings, query=query, filters=active_filters)